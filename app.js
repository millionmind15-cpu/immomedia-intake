(function () {
  const root = document.documentElement;
  const themeButton = document.querySelector('[data-theme-toggle]');
  let theme = window.matchMedia('(prefers-color-scheme: dark)').matches ? 'dark' : 'light';
  root.setAttribute('data-theme', theme);

  themeButton?.addEventListener('click', () => {
    theme = theme === 'dark' ? 'light' : 'dark';
    root.setAttribute('data-theme', theme);
    themeButton.setAttribute('aria-label', theme === 'dark' ? 'Helle Darstellung aktivieren' : 'Dunkle Darstellung aktivieren');
  });

  const form = document.querySelector('#intake-form');
  const preview = document.querySelector('#payload-preview');
  const result = document.querySelector('[data-result]');
  const customerResult = document.querySelector('[data-customer-result]');
  const statusPill = document.querySelector('[data-status-pill]');
  const fileInputs = Array.from(document.querySelectorAll('input[type="file"][data-document-type]'));
  const fileList = document.querySelector('[data-file-list]');
  const objectCatalog = Array.isArray(window.IMMOMEDIA_OBJECTS) ? window.IMMOMEDIA_OBJECTS : [];
  const objectSelect = form.elements.namedItem('objectId');
  const objectSummary = document.querySelector('[data-object-summary]');

  const params = new URLSearchParams(window.location.search);
  const debugMode = params.get('debug') === '1';
  const initialRequestId = params.get('requestId') || `REQ-${new Date().toISOString().slice(0, 10).replaceAll('-', '')}-${Math.random().toString(36).slice(2, 7).toUpperCase()}`;

  if (!debugMode) {
    document.querySelectorAll('[data-internal-only]').forEach((element) => {
      element.hidden = true;
    });
  }

  const demo = {
    requestId: initialRequestId,
    customerEmail: 'kunde@example.de',
    objectId: 'OBJ-FUTURIA-GARCHING',
    address: 'FUTURIA GARCHING, Garching bei München',
    objectType: 'Neubauprojekt',
    price: '850000',
    livingArea: '124',
    contactName: 'Max Mustermann',
    contactEmail: 'kunde@example.de',
    leadName: 'Interessent GmbH',
    leadPhone: '+49 30 123456',
    objectStatus: 'Im Hochbau',
    priority: 'Hoch',
    notes: 'Bitte die Unterlagen dem ausgewählten Objekt zuordnen und nach interner Prüfung weiterverarbeiten.'
  };

  function escapeHtml(value) {
    return String(value || '')
      .replaceAll('&', '&amp;')
      .replaceAll('<', '&lt;')
      .replaceAll('>', '&gt;')
      .replaceAll('"', '&quot;')
      .replaceAll("'", '&#039;');
  }

  function setField(name, value) {
    const field = form.elements.namedItem(name);
    if (field) field.value = value || '';
  }

  function getObjectById(id) {
    return objectCatalog.find((item) => item.id === id) || null;
  }

  function populateObjectSelect() {
    if (!objectSelect) return;
    const activeObjects = objectCatalog.filter((item) => item.formActive);
    objectSelect.innerHTML = '<option value="">Bitte Projekt wählen</option>';
    activeObjects.forEach((item) => {
      const option = document.createElement('option');
      option.value = item.id;
      option.textContent = `${item.name} · ${item.location} · ${item.status}`;
      objectSelect.append(option);
    });
  }

  function selectedObject() {
    return getObjectById(objectSelect?.value);
  }

  function renderObjectSummary(object) {
    if (!objectSummary) return;
    if (!object) {
      objectSummary.textContent = 'Wähle ein Immomedia Objekt aus, damit n8n Objektstatus und Zielordner eindeutig zuordnen kann.';
      return;
    }
    objectSummary.innerHTML = `
      <strong>${escapeHtml(object.name)}</strong>
      <span>${escapeHtml(object.location)} · ${escapeHtml(object.status)}</span>
      <span>Objekt-ID: ${escapeHtml(object.id)}</span>
      <span>Der interne Versandweg wird nach dem Upload automatisch über die Objekt-ID zugeordnet.</span>
    `;
  }

  function applyObjectSelection() {
    const object = selectedObject();
    if (!object) {
      setField('objectStatus', '');
      renderObjectSummary(null);
      return null;
    }
    setField('address', `${object.name}, ${object.location}`);
    form.elements.namedItem('address').dataset.objectId = object.id;
    setField('objectType', 'Neubauprojekt');
    setField('objectStatus', object.status || '');
    renderObjectSummary(object);
    return object;
  }

  function seedFromUrl() {
    const requestField = form.elements.namedItem('requestId');
    if (requestField) requestField.value = initialRequestId;
    const customerEmail = params.get('customerEmail');
    const objectId = params.get('objectId');
    const webhookUrl = params.get('webhookUrl');
    if (customerEmail && form.elements.namedItem('customerEmail')) form.elements.namedItem('customerEmail').value = customerEmail;
    if (customerEmail && form.elements.namedItem('contactEmail')) form.elements.namedItem('contactEmail').value = customerEmail;
    if (objectId && objectSelect) {
      objectSelect.value = objectId;
      applyObjectSelection();
    }
    if (webhookUrl && form.elements.namedItem('webhookUrl')) form.elements.namedItem('webhookUrl').value = webhookUrl;
  }

  function numberOrNull(value) {
    if (!value) return null;
    const normalized = String(value).replace(',', '.').replace(/[^\d.]/g, '');
    const parsed = Number(normalized);
    return Number.isFinite(parsed) ? parsed : null;
  }

  function formData() {
    return Object.fromEntries(new FormData(form).entries());
  }

  function selectedFiles() {
    return fileInputs.flatMap((input) =>
      Array.from(input.files || []).map((file) => ({
        file,
        documentType: input.dataset.documentType,
        fieldName: input.dataset.documentType || 'documents'
      }))
    );
  }

  function documentMetadata() {
    return selectedFiles().map(({ file, documentType }) => ({
      type: documentType,
      name: file.name,
      size_bytes: file.size,
      mime_type: file.type || 'application/octet-stream'
    }));
  }

  function buildPayload() {
    const data = formData();
    const [streetPart, cityPart] = (data.address || '').split(',').map((item) => item.trim());
    const object = selectedObject();
    const objectId = object?.id || data.objectId || form.elements.namedItem('address')?.dataset.objectId || `manual-${Date.now().toString().slice(-6)}`;
    return {
      event: 'manual_intake_submitted',
      source: 'immomedia_webform',
      submitted_at: new Date().toISOString(),
      request_id: data.requestId || initialRequestId,
      property: {
        id: objectId,
        catalog_id: objectId,
        name: object?.name || '',
        location: object?.location || '',
        address: data.address || '',
        street: streetPart || '',
        city: cityPart || ''
      },
      customer_access: {
        customer_email: data.customerEmail || data.contactEmail || '',
        customer_link: buildCustomerLink(false)
      },
      contact: {
        name: data.contactName || '',
        email: data.contactEmail || ''
      },
      lead: {
        name: data.leadName || '',
        phone: data.leadPhone || ''
      },
      workflow: {
        priority: data.priority || 'Normal',
        pdf_status: 'incoming',
        storage_path: `/Immomedia/${objectId}/${data.requestId || initialRequestId}`,
        notes: data.notes || ''
      },
      documents: documentMetadata()
    };
  }

  function buildCustomerLink(includeWebhook = false) {
    const data = formData();
    const url = new URL(window.location.href);
    url.search = '';
    url.searchParams.set('requestId', data.requestId || initialRequestId);
    const customerEmail = data.customerEmail || data.contactEmail;
    if (customerEmail) url.searchParams.set('customerEmail', customerEmail);
    const objectId = data.objectId || form.elements.namedItem('address')?.dataset.objectId;
    if (objectId) url.searchParams.set('objectId', objectId);
    if (includeWebhook && data.webhookUrl) url.searchParams.set('webhookUrl', data.webhookUrl);
    return url.toString();
  }

  function renderPreview() {
    const payload = buildPayload();
    if (preview) preview.textContent = JSON.stringify(payload, null, 2);
    return payload;
  }

  function setResult(message, state = '') {
    const targets = [result, customerResult].filter(Boolean);
    if (!targets.length) return;
    targets.forEach((target) => {
      target.textContent = message;
      const baseClass = target.hasAttribute('data-customer-result') ? 'result customer-result' : 'result';
      target.className = `${baseClass} ${state}`.trim();
    });
  }

  function setStatus(message, isError = false) {
    statusPill.textContent = message;
    statusPill.classList.toggle('error', isError);
  }

  function renderFiles() {
    const files = selectedFiles();
    if (!files.length) {
      fileList.textContent = 'Noch keine Unterlagen ausgewählt.';
      return;
    }
    const items = files
      .map(({ file, documentType }) => `<li>${documentType}: ${file.name} · ${(file.size / 1024 / 1024).toFixed(2)} MB</li>`)
      .join('');
    fileList.innerHTML = `<ul>${items}</ul>`;
  }

  function validateForm() {
    const required = ['requestId', 'objectId', 'address', 'objectType', 'contactName', 'contactEmail'];
    const data = formData();
    const missing = required.filter((field) => !String(data[field] || '').trim());
    const emailFields = ['customerEmail', 'contactEmail'];
    const invalidEmails = emailFields.filter((field) => data[field] && !/^[^\s@]+@[^\s@]+\.[^\s@]+$/.test(data[field]));
    const requiredDocuments = ['id_document', 'financing_confirmation', 'reservation_confirmation'];
    const presentDocumentTypes = new Set(selectedFiles().map((entry) => entry.documentType));
    const missingDocuments = requiredDocuments.filter((type) => !presentDocumentTypes.has(type));
    const missingConsent = selectedFiles().length > 0 && !form.elements.namedItem('consent')?.checked;
    if (missing.length || invalidEmails.length || missingConsent || missingDocuments.length) {
      setStatus('Bitte prüfen', true);
      return {
        ok: false,
        message: `Bitte Pflichtfelder ausfüllen${invalidEmails.length ? ' und E-Mail-Adressen prüfen' : ''}${missingDocuments.length ? ' sowie Ausweis, Finanzierungsbestätigung und Reservierungsbestätigung hochladen' : ''}${missingConsent ? ' und Upload-Zustimmung bestätigen' : ''}.`
      };
    }
    setStatus('Valide', false);
    return { ok: true };
  }

  form?.addEventListener('input', () => {
    renderFiles();
    renderPreview();
    validateForm();
  });

  objectSelect?.addEventListener('change', () => {
    applyObjectSelection();
    renderPreview();
    validateForm();
  });

  fileInputs.forEach((input) => input.addEventListener('change', () => {
    renderFiles();
    renderPreview();
    validateForm();
  }));

  document.querySelector('[data-action="fill-demo"]')?.addEventListener('click', () => {
    Object.entries(demo).forEach(([key, value]) => {
      const field = form.elements.namedItem(key);
      if (field) field.value = value;
    });
    applyObjectSelection();
    renderPreview();
    validateForm();
    setResult('Demo-Daten wurden eingesetzt. Du kannst jetzt JSON kopieren oder an n8n senden.', 'success');
  });

  document.querySelector('[data-action="copy-json"]')?.addEventListener('click', async () => {
    const payload = renderPreview();
    try {
      await navigator.clipboard.writeText(JSON.stringify(payload, null, 2));
      setResult('JSON wurde in die Zwischenablage kopiert.', 'success');
    } catch (error) {
      setResult('Kopieren nicht möglich. Bitte den JSON-Block manuell markieren.', 'error');
    }
  });

  document.querySelector('[data-action="copy-customer-link"]')?.addEventListener('click', async () => {
    const link = buildCustomerLink(false);
    try {
      await navigator.clipboard.writeText(link);
      setResult('Kundenlink wurde kopiert. Diesen Link per Outlook an den Kunden senden.', 'success');
    } catch (error) {
      setResult(`Kundenlink: ${link}`, 'success');
    }
  });

  form?.addEventListener('submit', async (event) => {
    event.preventDefault();
    const validation = validateForm();
    const payload = renderPreview();
    if (!validation.ok) {
      setResult(validation.message, 'error');
      return;
    }

    const data = formData();
    if (!data.webhookUrl) {
      setResult('Dry-Run erfolgreich. Kein n8n Endpoint gesetzt, daher wurde nichts gesendet.', 'success');
      return;
    }

    setResult('Sende Payload an n8n ...');
    try {
      const body = new FormData();
      body.append('payload', JSON.stringify(payload));
      selectedFiles().forEach(({ file, documentType }) => {
        body.append(`document_${documentType}`, file, file.name);
      });
      const response = await fetch(data.webhookUrl, {
        method: 'POST',
        headers: {
          'X-Immomedia-Mock-Source': 'webform'
        },
        body
      });
      const text = await response.text();
      if (!response.ok) {
        throw new Error(`n8n antwortete mit HTTP ${response.status}: ${text.slice(0, 240)}`);
      }
      setResult(`Gesendet. n8n Antwort: ${text || response.status}`, 'success');
    } catch (error) {
      setResult(`Senden fehlgeschlagen: ${error.message}`, 'error');
    }
  });

  populateObjectSelect();
  seedFromUrl();
  if (!selectedObject() && objectCatalog.length) renderObjectSummary(null);
  renderFiles();
  renderPreview();
  validateForm();
})();
